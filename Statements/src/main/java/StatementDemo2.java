...
public class StatementDemo2 {
    ...
    // URL de conexion a la base de datos futbol HSQLDB
    ...
    // En este caso vamos a utilizar un bloque try-with-resources para la conexion y el statement
    // De este modo no será necesario cerrarlos explicitamente en un bloque finally
        ...
        try (Connection conn = DriverManager.getConnection(HSQLDB_FUTBOL_URL)) {
            try (Statement stmt = conn.createStatement()) {
                try {
                    ...
                    generatedKeys.next(); // Posicionar el cursor en la primera fila de resultados
